...
def findNBCLhosts():
  ...
  while True:
    ...
    if checkwlan:
      ...
      n_wlan_time = re.findall('[0-9]+\.[0-9]+s', sl.strip())
      neighbor_list.append([n_wlan_MAC[0], n_wlan_time[0], []])
  ...
  return (neighbor_list, apclient_list)
...
def meshinfo_thread():
    ...
    try:
        ...
        while True:
            ...
            print("meshinfo obj data to be sent to base:")
            for key in obj.keys():
                print(key, ":", obj[key])
            print('meshinfo obj length: ' + str(l))
            # send 2byte_len + payload to mobile
            baseclientSocket.send(l.to_bytes(2, byteorder='big') + data.encode())
            print('Mesh Info data sent to Base Station %s successfully\n' % str(baseclientSocket.getpeername()))
            count += 1
            time.sleep(3)
